...
int main()
{
  ...
  do
  {
    ...
    switch(ch)
    {
       ...
       scanf("%s",txt);
       if((fd1=open(txt,O_RDONLY, 0777))<0)
       {
          printf("Error in opening the file\n");
          break;
       }
       ...
       printf("Enter the File name to open\n");
       scanf("%s",txt);
       if((fd1=open(txt,O_WRONLY| O_APPEND, 0777))<0)
       {
          printf("Error in opening the file\n");
          break;
       }
       ...
       {
       	   if((lseek(fd1,5,SEEK_SET))<0)
           {
	      printf("Error in pointing to the offset bytes\n");
              break;
           }
           else
	   {
