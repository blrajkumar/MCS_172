...
int main()
{
    ...
    while (c_conct)
    {
        ...
        char ch;
        //Menu Received
        valread = read(sock, info_buff, 1024); //Read 1
        printf("%s\n", info_buff);
        ...
        valread = read(sock, info_buff, 1024); //Read 2
        printf("%s\n", info_buff);
        memset(info_buff, 0, sizeof(info_buff));
        ...
        valread = read(sock, info_buff, 1024); //Read 3
        printf("%s\n", info_buff);
        memset(info_buff, 0, sizeof(info_buff));

        //Reading - if to add more pizza to cart
        valread = read(sock, info_buff, 1024); //Read 4
        printf("%s\n", info_buff);
        memset(info_buff, 0, sizeof(info_buff));
        ...
        else
        {
            c_conct = false;
            //Reading - end note
            valread = read(sock, info_buff, 1024); //Read end 1
            printf("%s\n", info_buff);
            memset(info_buff, 0, sizeof(info_buff));

            valread = read(sock, info_buff, 1024); //Read end 2
            printf("%s\n\n", info_buff);
            memset(info_buff, 0, sizeof(info_buff));
            memset(info_to_server, 0, sizeof(info_to_server));
        }
